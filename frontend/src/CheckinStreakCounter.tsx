...
export function CheckinStreakCounter(props: Props) {
  ...
  const [displayValue, setDisplayValue] = useState(props.streak);
  const [rolling, setRolling] = useState(false);

  useEffect(() => {
    const from = props.animateFrom;
    if (from != null && from !== props.streak && props.streak > from) {
      setDisplayValue(from);
      setRolling(true);
      const id = window.requestAnimationFrame(() => {
        setDisplayValue(props.streak);
      });
    ...
    setDisplayValue(props.streak);
    setRolling(false);
  }, [props.streak, props.animateFrom]);
  ...
  return (
    ...
    >
      ...
      <span className="trello-checkin-streak-value-wrap" aria-hidden>
        <span className="trello-checkin-streak-value">{displayValue}</span>
      </span>
